...
#        print(f"{os.path.splitext(self.fout)[0]}")
#        print(f"{self.fnm}")
#        exit()
...
#        exit()
        os.system(f"apktool b -f -d {os.path.splitext(self.fout)[0]}")
        print("\x1b[1;92m[+] Signing APK file... \x1b[0m",end="")
        os.system(f"apksigner sign --ks assets/user.keystore --ks-key-alias user --ks-pass pass:12345678 {os.path.splitext(self.fout)[0]}/dist/{self.fnm}")
        print("\x1b[1;92mOK\x1b[0m")
        print("\x1b[1;92m[+] Verifying APK file... \x1b[0m",end="")
        os.system(f"apksigner verify {os.path.splitext(self.fout)[0]}/dist/{self.fnm}")
        print("\x1b[1;92mOK\x1b[0m")
        ...
        os.rename(os.path.splitext(os.path.splitext(self.fout)[0])[0]+"/dist/"+self.fnm, self.signed)
        # Delete Project if isclean = True
        if self.isclean: os.system(f"rm -rf {os.path.splitext(self.fout)[0]}")
    ...
    def writeManifestXML(self):
        if len(self.manifestxml.split("</manifest>")) > 1:
            self.manifestxml = self.manifestxml.split("</manifest>")[0]+"</manifest>"
        open(os.path.splitext(self.fout)[0]+"/AndroidManifest.xml","w").write(self.manifestxml)
    def writeNeutralize(self):
        if not self.isneutralize:
            os.system(f"mkdir -p {os.path.splitext(self.fout)[0]}/smali/sec/blackhole/dtlx")
            open(os.path.splitext(self.fout)[0]+"/smali/sec/blackhole/dtlx/Schadenfreude.smali","w").write(neutralize)
    ...
    def removeCopyProtection(self):
        ...
        self.f_ls = list(map(lambda x: os.path.splitext(self.fout)[0]+"/"+x,self.f_ls))
        ...
        self.f_ls = list(filter(lambda x: x[0].startswith(os.path.splitext(self.fout)[0]+"/smali"), list(__import__("itertools").permutations(self.smalidir+["andhook","com/applisto","com/swift/sandhook"], 2))))
        ...
        for iterx in self.smalidir:
            ...
            list(map(lambda x: self.warning(x), tmp_fls))
        # Clean apktool.yml
        self.f_ls = list(filter(lambda x: not "com/applisto" in x, open(os.path.splitext(self.fout)[0]+"/apktool.yml","r").read().split(self.endl)))
        self.f_ls = list(filter(lambda x: x.strip() != "", self.f_ls))
        open(os.path.splitext(self.fout)[0]+"/apktool.yml","w").write("\012".join(self.f_ls))
    ...
    def removeProperties(self):
        ...
        self.f_ls = subprocess.run(f"find {os.path.splitext(self.fout)[0]}/unknown/",shell=True,check=True,stdout=subprocess.PIPE).stdout.decode().split(self.endl)
        ...
        list(map(lambda x: self.warning(x), tmp_fls))
        # Clean apktool.yml
        self.f_ls = open(os.path.splitext(self.fout)[0]+"/apktool.yml","r").read().split(self.endl)
        self.f_ls = list(filter(lambda x: x.strip() != "", self.f_ls))
        tmp_fls = []
        self.modifiedsourcefile = ""
        ...
        list(map(lambda x: self.warning(x), tmp_fls))
        open(os.path.splitext(self.fout)[0]+"/apktool.yml","w").write(self.modifiedsourcefile)
...
strmatch = {
    "adloader": (
        "loadAd",
#        "load", # Neutralizes too much
#        "show", # Neutralizes too much
        "loadAdFromBid",
        "loadAds",
        "requestBannerAd",
        ...
        "loadInterstitialAd",
        "loadNativeAd",
        "startActivity" # Added - does ad loading as well
    )
}
trackers = [
